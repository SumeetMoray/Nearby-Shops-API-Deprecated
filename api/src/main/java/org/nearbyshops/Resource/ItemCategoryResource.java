...
public class ItemCategoryResource {
	...
	public Response getAllItemCategory(
			...
			@QueryParam("lat.max")int latmax,@QueryParam("lat.min")int latmin,
			@QueryParam("lon.max")int lonmax,@QueryParam("lon.min")int lonmin)
	
	
	{	
		List<ItemCategory> list = 
				Globals.itemCategoryService.readAllItemCategories(
						shopID,parentID,
						latmax,latmin,
						lonmax,lonmin);
